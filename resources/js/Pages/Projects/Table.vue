    ...
    <div class="h-full" :class="{ 'right_menu_enable': show_right_menu }">
        ...
        <div class="flex flex-col flex-grow-1 flex-shrink-1 h-full">
            <board-view-menu :project="project" @filter-toggle="open_filter = !open_filter"
                @menu-toggle="show_right_menu = !show_right_menu" @fClear="reset()" :filters="filters" view="table" />
                ...
                <div class="inline-block min-w-full h-full py-4 align-middle md:px-3 lg:px-4">
                    <div class="table__view">
                        ...
                        <div class="absolute flex w-[300px] z-10 text-sm flex-col bg-white px-4 py-4 rounded shadow"
                            ...
                            <input v-model="list_search" class="border-[2px] px-2 py-1 border-gray-400 rounded-[3px]"
                            ...
                            <ul class="flex flex-col mt-3 gap-1 h-48 max-h-[200px] overflow-y-auto">
                                <li v-for="(listObject, li_index) in searchList(list_search)">
                                    ...
                                    <label class="flex p-2 cursor-pointer hover:bg-gray-200 rounded">
                                        <input name="task_list" class="w-5 ml-1 mr-2" type="radio"
                                            :checked="isCurrentLabel(listObject.id)"
                                            @change="saveTask(selected.task_id, { list_id: listObject.id }, selected.list_index)">
                                        <span data-a="" class="p-1" type="button" :tabindex="li_index">{{
                                            listObject.title }}</span>
                                    </label>
                                </li>
                            </ul>
                        </div>
                        ...
                        <!-- Label Search -->
                        <div class="absolute flex w-[300px] z-10 text-sm flex-col bg-white px-4 py-4 rounded shadow"
                            :style="{ top: selected.top, left: selected.left }" v-if="showLabelBox">
                                ...
                                <li v-for="(lab, lab_index) in searchLabel(label_search)">
                                    <label class="flex gap-1">
                                        ...
                                        <span class="w-full px-3 py-2 rounded cursor-pointer hover:opacity-80"
                                            :style="{ background: lab.color }" :tabindex="lab_index"
        ...
        <task-details v-if="taskDetailsOpen" :id="taskDetailsId" view="table" :isPopup="td_pop"

  ...
  <div v-if="workspace" class="sidebar_wrapper">
    <div class="p-3 flex flex-wrap gap-2 items-center relative">
      ...
      <Link
        ...
        class="name flex flex-wrap w-[140px] text-ellipsis leading-5 cursor-pointer"
      >
        {{ workspace.name }}
      </Link>
    ...
    <ul class="font-medium text-sm items">
      <!-- Workspace Task (solo admin del workspace) -->
      <li v-if="isAdmin">
        <Link
          :href="route('workspace.tables', workspace.slug || workspace.id)"
          class="flex items-center px-3 py-2 dark:text-white hover:bg-gray-200 dark:hover:bg-gray-700 group"
          :class="{
            active:
              checkActiveClass('component', 'Workspaces_Table') &&
              !($page?.props?.filters && $page?.props?.filters?.user),
          }"
        >
      ...
      <!-- My Tasks (solo si hay usuario logueado disponible en props) -->
      <li v-if="authUserId">
        <Link
          :href="route('workspace.tables', { uid: workspace.slug || workspace.id, user: authUserId })"
          class="flex items-center px-3 py-2 dark:text-white hover:bg-gray-200 dark:hover:bg-gray-700 group"
          :class="{ active: checkActiveClass('filter') }"
        >
      ...
      <!-- Team Members (solo admin del workspace) -->
      ...
      <li>
        <Link
          :href="route('workspace.backlog', { uid: workspace.slug || workspace.id })"
          class="flex items-center px-3 py-2 dark:text-white hover:bg-gray-200 dark:hover:bg-gray-700 group"
          :class="{ active: checkActiveClass('component', 'Workspaces_Backlog') }"
        >
          <icon class="w-4 h-4" name="list" />
      ...
      <!-- Estadísticas generales (solo admin y CLT por slug de rol global) -->
      <li v-if="canSeeGeneralStats">
        <Link
          :href="route('workspace.statistics.general', workspace.slug || workspace.id)"
          class="flex items-center px-3 py-2 dark:text-white hover:bg-gray-200 dark:hover:bg-gray-700 group"
          :class="{ active: checkActiveClass('component', 'Workspaces_StatisticsGeneral') }"
        >
          <icon class="w-4 h-4" name="dashboard" />
          <span class="ml-3">{{ __('Estadísticas generales') }}</span>
        </Link>
      </li>
    </ul>
    ...
    >
      <li v-for="(project, p_index) in favorites" :key="project.id || p_index" class="flex group">
        <Link
    ...
    <div class="flex text-[13px] text items-center justify-between mt-4 font-bold px-2 pt-2 border-t border-[#ffffff29]">
      ...
      <div class="flex">
        ...
        <!-- Crear proyecto (solo admin del workspace) -->
        <div
          v-if="isAdmin"
    ...
    >
      <li v-for="(project, p_index) in projects" :key="project.id || p_index" class="flex group">
        <Link
          :href="route('projects.view.board', project.slug || project.id)"
          class="p-2 relative block w-full item"
          :class="{ active: project.id === ($page?.props?.project ? $page.props.project.id : '') }"
        >
          <div class="flex h-5 relative">
            <div
              v-if="project.background"
              :style="{ 'background-image': 'url(' + project.background.image + ')' }"
              class="flex bg-cover rounded-full w-5 h-5 border"
            ></div>
...
export default {
  ...
  computed: {
    ...
    rolSlug() {
      return this.$page?.props?.auth?.user?.role?.slug || this.$page?.props?.auth?.user?.rol?.slug || null
    },

    canSeeGeneralStats() {
      return ['admin', 'CLT'].includes(this.rolSlug)
    },
      ...
      handler() {
        if (this.$page?.props?.project) {
          if (this.$page.props.project.workspace.id !== this.workspace?.id) {
            ...
            if (projectIndex !== -1) this.projects[projectIndex] = this.$page.props.project
          }
  ...
  methods: {
    checkActiveClass(type, name) {
      if (type === 'filter') {
        const filterUser = this.$page?.props?.filters?.user
        const authId = this.authUserId
        if (!filterUser || !authId) return ''
        return parseInt(filterUser, 10) === parseInt(authId, 10) ? 'active' : ''
      }

      if (type === 'component') {
        const comp = this.$page?.component
        if (!comp) return ''
        return comp.replace('/', '_') === name ? 'active' : ''
      }
    ...
    saveProject(e, project) {
      e.preventDefault()
      axios.post(this.route('json.p.starred.save', project.id)).then(() => {
        this.getProjects()
        this.getStarredProjects()
      })
    },
